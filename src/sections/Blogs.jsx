          ...
          <AnimatePresence mode="popLayout">
            {BLOGS.map((blog, i) => (
              ...
              >
                ...
                >
                  {/* Left: Image */}
                  <div className={`relative w-full md:w-72 lg:w-80 h-48 sm:h-56 md:h-60 lg:h-64 shrink-0 rounded-xl overflow-hidden ${blog.imageFit === "contain" ? "bg-[#0b0b0f]" : `bg-gradient-to-br ${blog.accent}`}`}>
                    ...
                    {blog.image ? (
                      <img src={blog.image} alt={blog.title} loading="lazy" decoding="async" className={`absolute inset-0 w-full h-full ${blog.imageFit === "contain" ? "object-contain" : "object-cover"} opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700 ease-out`} />
                    ) : (
